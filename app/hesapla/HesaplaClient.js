...
export default function HesaplaClient({ brandsData }) {
  ...
  const brands = useMemo(() => Object.entries(brandsData).sort((a, b) => a[1].name.localeCompare(b[1].name, 'tr')), [brandsData]);

  const models = useMemo(() => {
    if (!brand) return [];
    return Object.entries(brandsData[brand]?.models || {}).sort((a, b) => a[1].name.localeCompare(b[1].name, 'tr'));
  }, [brand, brandsData]);

  const variants = useMemo(() => {
    if (!brand || !model) return [];
    return Object.entries(brandsData[brand]?.models[model]?.variants || {}).sort((a, b) => a[1].name.localeCompare(b[1].name, 'tr'));
  }, [brand, model, brandsData]);

  const hasVariants = variants.length > 0;

  const years = useMemo(() => {
    if (!brand || !model) return [];
    const m = brandsData[brand]?.models[model];
    if (!m) return [];
    if (hasVariants) {
      if (!variant) return [];
  ...
  }, [brand, model, variant, hasVariants, brandsData]);
  ...
  }, [brand, model, year, variant, hasVariants, brandsData]);
  ...
  const selectedVehicle = brand && model && year && fuel ? {
    brand: brandsData[brand]?.name,
    model: brandsData[brand]?.models[model]?.name,
    variant: variant ? brandsData[brand]?.models[model]?.variants[variant]?.name : null,
    year,
    fuel: YAKIT_TURLERI[fuel],
    consumption: hasVariants
      ? getYearData(brandsData[brand]?.models[model]?.variants[variant]?.years, year)?.consumption[fuel]
      : getYearData(brandsData[brand]?.models[model]?.years, year)?.consumption[fuel],
  } : null;
        ...
        <div className={styles.layout}>
          ...
          <div className={styles.formArea}>
            <div className={styles.formHeader}>
              <h1 className={`${styles.title} font-display anim-fade-up`}>Maliyet Hesapla</h1>
              <p className={`${styles.subtitle} anim-fade-up d1`}>Aracınızı seçin, sonuçları anında görün.</p>
            </div>
