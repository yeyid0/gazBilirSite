...
function VehicleSelector({ id, label, selected, onChange, locked, overrides, onOverridesChange, brandsData }) {
  const [showAdvanced, setShowAdvanced] = useState(false);

  const brands = useMemo(() =>
    Object.entries(brandsData).sort((a, b) => a[1].name.localeCompare(b[1].name, 'tr')), [brandsData]);

  const models = useMemo(() => {
    if (!selected.brand) return [];
    return Object.entries(brandsData[selected.brand]?.models || {})
      .sort((a, b) => a[1].name.localeCompare(b[1].name, 'tr'));
  }, [selected.brand, brandsData]);

  const modelData = useMemo(() =>
    selected.brand && selected.model
      ? brandsData[selected.brand]?.models[selected.model]
      : null,
  [selected.brand, selected.model, brandsData]);
    ...
    <div className={`${styles.vehicleCol} ${locked ? styles.vehicleColLocked : ''}`}>
      ...
      <div className={styles.formGrid}>
        <div className="form-group">
          ...
          <select className="form-select" value={selected.brand} onChange={(e) => onChange({ ...EMPTY, brand: e.target.value })} disabled={locked}>
        ...
        <div className="form-group">
          ...
          <select className="form-select" value={selected.model} onChange={(e) => onChange({ ...EMPTY, brand: selected.brand, model: e.target.value })} disabled={locked || !selected.brand}>
        ...
        {hasVariants && (
          <div className="form-group">
            ...
            <select className="form-select" value={selected.variant} onChange={(e) => onChange({ ...EMPTY, brand: selected.brand, model: selected.model, variant: e.target.value })} disabled={locked || !selected.model}>
        ...
        <div className={styles.row}>
          <div className="form-group">
            ...
            <select className="form-select" value={selected.year} onChange={(e) => onChange({ ...selected, year: e.target.value, fuel: '' })} disabled={locked || (hasVariants ? !selected.variant : !selected.model)}>
          ...
          <div className="form-group">
            ...
            {fuelTypes.length === 1 ? (
              ...
              <select className="form-select" value={selected.fuel} onChange={(e) => onChange({ ...selected, fuel: e.target.value })} disabled={locked || !selected.year}>
      ...
      {locked && (
        <button className={styles.unlockBtn} onClick={() => onChange({ ...EMPTY })}>
          Değiştir
        </button>
      )}
      ...
      <div className={styles.advancedSection}>
        ...
        >
          ...
          Özel Değerler
          {overrideCount > 0 && <span className={styles.advancedCount}>{overrideCount} aktif</span>}
        </button>
...
function KarsilastirContent({ brandsData }) {
  ...
  const handleCarAChange = (val) => { setCarALocked(false); setCarA(val); setOverridesA({}); };
  const handleCarBChange = (val) => { setCarB(val); setOverridesB({}); };

  const calculateCar = useCallback((car, overrides = {}) => {
    if (!car.brand || !car.model || !car.year || !car.fuel) return null;
    const mData = brandsData[car.brand]?.models[car.model];
  ...
  const getDisplayName = (car) => {
    const b = brandsData[car.brand]?.name || '';
    const m = brandsData[car.brand]?.models[car.model]?.name || '';
    const v = car.variant ? brandsData[car.brand]?.models[car.model]?.variants?.[car.variant]?.name || '' : '';
    return `${b} ${m}${v ? ' ' + v : ''}`.trim();
  };
    ...
    <div className={styles.page}>
      ...
      <div className="container">
        ...
        <div className={`${styles.globalOptions} anim-fade-up d1`}>
          ...
          <input type="number" className="form-input" value={km} onChange={(e) => setKm(Number(e.target.value))} min="100" max="10000" step="100" style={{ textAlign: 'center', fontSize: 'var(--text-xl)' }} />
          <div className={styles.presets}>
            {presets.map((p) => (
              <button key={p} type="button" className={`${styles.preset} ${km === p ? styles.presetActive : ''}`} onClick={() => setKm(p)}>
        ...
        <div className={`${styles.setupGrid} anim-fade-up d2`}>
          <VehicleSelector id="A" label="1. Araç Seçimi" selected={carA} onChange={handleCarAChange} locked={carALocked} overrides={overridesA} onOverridesChange={setOverridesA} brandsData={brandsData} />
          <VehicleSelector id="B" label="2. Araç Seçimi" selected={carB} onChange={handleCarBChange} locked={false} overrides={overridesB} onOverridesChange={setOverridesB} brandsData={brandsData} />
        </div>
        ...
        {(resultA || resultB) && (
          <div className={styles.resultsArea}>
            ...
            <div className={styles.compareTable}>
              ...
              <div className={styles.cRowTotal}>{renderValue(resultA?.total, resultB?.total, 'total')}</div>
              <div className={styles.cBreakdown}>
                {MALIYET_KALEMLERI.map((item) => renderValue(resultA?.breakdown[item.key], resultB?.breakdown[item.key], item.key))}
              </div>
            </div>
          </div>
        )}
...
export default function KarsilastirClient({ brandsData }) {
  return (
    <Suspense fallback={<div style={{ textAlign: 'center', padding: '8rem 0', color: 'var(--text-muted)' }}>Yükleniyor...</div>}>
      <KarsilastirContent brandsData={brandsData} />
    </Suspense>
  );
}
